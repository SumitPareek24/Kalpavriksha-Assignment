...
int evaluateExpression1(char expr[], char result[])
{
...
int evaluateExpression2(char expr[])
{
    int result = 0;
    int currentNumber = 0;
    char lastOperator = '+';
...
bool checkValidation(char expr[]) {
    bool check = false;
    int j;
    for (j = 0; expr[j] != '\0'; j++)
    {
...
bool validateDoubleOperator(char expr[]) {
    char lastOpera = '\0';
...
int main()
{
    ...
    if(validateDoubleOperator(expr)) {
        printf("Error: Invalid expression\n");
        return 0;
    }

    int validate = evaluateExpression1(expr, result);
    ...
    int finalResult = evaluateExpression2(result);
